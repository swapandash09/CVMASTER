...
(function () {
  ...
  window.downloadPDF = function () {
    ...
    // Reset any preview zoom/transform so the export captures true A4 dimensions
    ...
    // Give the browser a tick to apply the transform reset before capture
    setTimeout(() => {
      html2pdf()
        .set(opt)
        .from(el)
        ...
        .then(() => {
          ...
          showToast('High quality PDF downloaded', 'success', 'file-pdf');
        })
        .catch((err) => {
          loadingOverlay.style.display = 'none';
          previewWrapper.style.transform = prevTransform;
